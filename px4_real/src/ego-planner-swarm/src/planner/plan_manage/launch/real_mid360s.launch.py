...
def generate_launch_description():
    ...
    fastlio_odom_relay_node = Node(
        package='topic_tools',
        executable='relay',
        name='relay_fastlio_odom_to_ego',
        arguments=['/Odometry', '/drone_0_visual_slam/odom'],
        output='screen'
    )

    fastlio_cloud_relay_node = Node(
        package='topic_tools',
        executable='relay',
        name='relay_fastlio_cloud_to_ego',
        arguments=['/cloud_registered', '/drone_0_pcl_render_node/cloud'],
        output='screen'
    )
    ...
    # 添加 Map Generator 节点
    ld.add_action(fastlio_odom_relay_node)
    ld.add_action(fastlio_cloud_relay_node)
